...
def go(args):
    ...
    # Convert integer columns with possible missing values to floats
    int_columns = X_val.select_dtypes(include=['int']).columns
    X_val[int_columns] = X_val[int_columns].astype(float)
    ...
    # Convert all 'object' type columns to 'str' to avoid issues with MLflow's infer_signature
    ...
    # Upload to W&B the feeture importance visualization
    run.log(
        {
